...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                             ...
                             QFileDialog, QLineEdit, QDialog, QCheckBox)
...
class CS1237_GUI(QMainWindow):
    ...
    def handle_adc_frame(self, data, timestamp):
        ...
        try:
            ...
            if "10 Hz" in rate_str: fs = 10.0
            elif "40 Hz" in rate_str: fs = 40.0
            elif "640 Hz" in rate_str: fs = 640.0
            elif "1280 Hz" in rate_str: fs = 1280.0
            else: fs = 10.0
            expected_interval = 1.0 / fs
        except Exception:
            expected_interval = 0.001 # 默认 1ms

        # 如果当前时间戳与上一个非常接近（小于理论间隔的一半），说明是批量到达
        # 此时强制递增时间戳，使其在图上展开
        if timestamp < self.last_frame_time + (expected_interval * 0.5):
            current_time = self.last_frame_time + expected_interval
        else:
            # 如果时间戳正常（有间隔），则使用测量时间，但不能回退
        ...
        if len(data) == 6: # 新的电压+PGA帧 (4+2=6字节)
            try:
                ...
                if self.current_channel_code == 2:  # 温度传感器
                    ...
                    self.log_message(f"🌡️ [{relative_time:7.2f}s] {temp_val:+.2f} °C (V={voltage_mv:.2f}mV)\n", category="adc")
                else:
                    # 直接使用电压值进行绘图（转换为 mV）
                    value_to_plot = voltage_value * 1000 # 转换为毫伏(mV)
                    self.log_message(f"📊 [{relative_time:7.2f}s] {voltage_value*1000:+.4f} mV (PGA×{pga_value})\n", category="adc")
            ...
            if self.current_channel_code == 2:  # 温度传感器
                ...
                value_to_plot = voltage_value * 1000 # 转换为毫伏(mV)
                self.log_message(f"📊 [{relative_time:7.2f}s] {voltage_value*1000:+.4f} mV (ADC={signed_value})\n", category="adc")
        ...
        try:
            with self.buffer_lock:
                self.buffered_points.append((relative_time, value_to_plot))
        ...
        if len(self.buffered_points) >= 5:
            try:
                while len(self.buffered_points) >= 5:  # 至少保留5个点进行五点法检测
                    ...
                    t_candidate, v_candidate = self.buffered_points[2]
                    ...
                    if len(prev_context) >= 2 and len(next_ctx_list) >= 2:
                        ...
                        if is_outlier_ctx:
                            ...
                            self.buffered_points[2] = (t_candidate, v_corrected)
                            
                            # 显示过滤信息
                            self.log_message(
                                f"🔧 [{t_candidate:7.2f}s] 过滤: {v_candidate:+.2f} → {v_corrected:+.2f} mV\n",
                                category="status"
                            )
                        
                        # 替换 p4（如果同时检测到p4也是异常）
                        if replacement_next is not None and len(self.buffered_points) > 3:
                            t_next = self.buffered_points[3][0]
                            self.buffered_points[3] = (t_next, replacement_next)
                            self.log_message(
                                f"🔧 [{t_next:7.2f}s] 过滤: {self.buffered_points[3][1]:+.2f} → {replacement_next:+.2f} mV (双点异常)\n",
                                category="status"
                            )
                    
                    # 输出第一个点（p1），它已经在之前的检测中被处理过
                    t_output, v_output = self.buffered_points[0]
                    self.recent_values.append(v_output)
                    self.plot_data_x.append(t_output)
    ...
    def clear_plot(self):
        ...
        try:
            ...
            self.small_canvas.flush_events()
        except Exception:
            pass
