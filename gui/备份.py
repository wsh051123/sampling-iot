...
class CS1237_GUI(QMainWindow):
    def __init__(self):
        ...
        self.min_points_before_plot = 50
    ...
    def is_outlier_in_context(self, value, local_window):
        ...
        # 要求至少有前后各4个点（共8个邻点）才能进行严格的数量级比较
        if len(local_window) < 8:
            return False
        ...
        # 对每个邻点检查是否与该点在数量级上差距很大（>=5x）
        # 如果对所有邻点都满足数量级差异（即 value 与每个邻点的比值 >= 5），
        # 则认为该点与周围点数量级不一样，判为异常。
        all_diff = True
        for n in local_window:
            abs_n = abs(n)
            # 如果邻点和当前点都非常接近0，则认为不满足数量级差异
                ...
                try:
                    ...
                    while len(self.buffered_points) > self.lookahead:
                        ...
                        if is_outlier_ctx:
                            ...
                            self.outlier_count += 1
                            try:
                                # 当确认为异常且已拥有前后各4点时，使用这8个点的均值作为替换值
                                if len(local_window) >= 8:
