...
class CS1237_GUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.temp_calib_btn.setStyleSheet("""
            QPushButton {
                background-color: #FF5722;
                color: white;
                font-weight: bold;
        ...
        if len(data) == 6: # 新的电压+PGA帧 (4+2=6字节)
            try:
                ...
                if self.current_channel_code == 2:  # 温度传感器
                    ...
                    self.log_message(f"📊 [{relative_time:7.2f}s] {value_to_plot:+.4f} mV (原始:{raw_mv:.2f}, PGA×{pga_value})\n", category="adc")
            ...
            if self.current_channel_code == 2:  # 温度传感器
                ...
                self.log_message(f"📊 [{relative_time:7.2f}s] {value_to_plot:+.4f} mV (原始:{raw_mv:.2f}, ADC={signed_value})\n", category="adc")
