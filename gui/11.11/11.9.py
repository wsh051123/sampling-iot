...
class SerialThread(QThread):
    """串口读取线程 - 支持协议帧解析"""
    data_received = pyqtSignal(str)  # 接收到文本数据的信号
    frame_received = pyqtSignal(int, bytes)  # 接收到帧的信号（命令，数据）
    ...
    def __init__(self, serial_port):
        ...
        self.running = True
        self.buffer = bytearray()  # 二进制缓冲区
        
        # 协议常量
        self.FRAME_HEAD = bytes([0xAA, 0x55])
        self.FRAME_TAIL = bytes([0x0D, 0x0A])
        self.CMD_ADC_DATA = 0x01
        self.CMD_ERROR = 0x03
        self.CMD_STATUS = 0x04
        self.CMD_CONFIG_ACK = 0xB1
        ...
        while self.running and self.serial_port and self.serial_port.is_open:
            try:
                if self.serial_port.in_waiting > 0:
                    ...
                    # 🔧 关键修复：先尝试解析二进制帧，只处理文本调试信息
                    # 添加到二进制缓冲区
                    self.buffer.extend(new_data)
                    
                    # 解析帧（会从buffer中移除已解析的帧）
                    self.parse_frames()
                    
                    # 处理非帧数据（调试文本）
                    # 只有在buffer中找不到有效帧后，才尝试解码为文本
                    # 并且只处理不包含二进制标记的纯文本数据
                    if len(self.buffer) > 0:
                        try:
                            # 检查buffer是否包含帧头
                            has_frame = self.buffer.find(self.FRAME_HEAD) != -1
                            
                            if not has_frame:
                                # buffer中没有帧头，尝试解码为文本
                                text = bytes(self.buffer).decode('utf-8', errors='ignore').strip()
                                if text and '\xaa' not in text and '\x55' not in text:
                                    # 确实是纯文本调试信息
...
class CS1237_GUI(QMainWindow):
    ...
    def on_data_received(self, line):
        ...
        if "RAW ADC" in line or "Voltage" in line:
            self.log_message("⚠️ 警告：检测到文本格式ADC数据！这不应该出现。\n")
    ...
    def handle_adc_frame(self, data):
        ...
        # 解析24位有符号ADC值（4字节传输，高位补0）
        adc_value = (data[1] << 16) | (data[2] << 8) | data[3]
        
        # 调试：显示原始字节（仅在控制台）
        print(f"DEBUG - ADC帧: [{data[0]:02X} {data[1]:02X} {data[2]:02X} {data[3]:02X}] → 原始值: {adc_value} (0x{adc_value:06X})")
        
        # 转换为有符号数
        if adc_value & 0x800000:
            signed_value = adc_value - 0x1000000
        else:
            signed_value = adc_value
        
        print(f"DEBUG - 有符号ADC值: {signed_value}")
        ...
        vref = 2.5  # 参考电压
        voltage = (signed_value / 8388608.0) * (vref / pga)
        ...
        # 显示每个数据到日志框
        self.log_message(f"📊 ADC: {signed_value:8d} | 电压: {voltage:+.8f} V | 总数: {self.total_received}\n")
                ...
                while len(self.buffered_points) > self.lookahead:
                    ...
                    try:
                        if abs(v_candidate) > 1000000:
                            self.log_message(f"DEBUG_CONTEXT: candidate={v_candidate}, prev={prev_context}, next={next_ctx_list}, local_len={len(local_window)}\n")
                    except Exception:
                        pass
                    ...
                    try:
                        if replacement_next is not None and len(self.buffered_points) > 1:
                            ...
                            try:
                                # 记录替换操作，便于排查
                                self.log_message(f"🔁 替换下一个缓冲点: 原值={old_next} -> 新值={replacement_next}")
                            except Exception:
                                pass
                    except Exception:
                        pass
                    ...
                    if is_outlier_ctx:
                        ...
                        try:
                            if replacement_value is not None:
                                ...
                                try:
                                    self.log_message(f"🔁 替换候选点: 原值={v_candidate} -> 新值={v_emit}")
                                except Exception:
                                    pass
    ...
    def handle_status_frame(self, data):
        ...
        pga_code = data[0]
        rate_code = data[1]
        success_count = (data[2] << 24) | (data[3] << 16) | (data[4] << 8) | data[5]
        
        pga_map = {0: 1.0, 1: 2.0, 2: 64.0, 3: 128.0}
        rate_map = {0: "10 Hz", 1: "40 Hz", 2: "640 Hz", 3: "1280 Hz"}
        
        pga = pga_map.get(pga_code, 128.0)
        rate = rate_map.get(rate_code, "10 Hz")
        
        self.log_message(f"📊 Arduino状态: PGA={pga}, 采样率={rate}, 成功读取={success_count}\n")
    ...
    def log_message(self, message):
        ...
        try:
            ...
            # 仅在用户当前正处于滚动底部附近时才自动滚动到最新（避免打断用户查看历史）
            try:
    ...
    def export_data_to_txt(self):
        ...
        try:
            ...
            self.log_message(f"✅ 数据已导出: {file_path} (同时导出 CSV)")
